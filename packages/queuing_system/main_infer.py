...
def worker(w: int, p: dict, queuing_simulator, output_dir: str, j: int, delay_rate, total_threads, iat, srvt):
    pid = int(mp.current_process().name.split('-')[1]) - 1
    max_play = p.get('max_play')
    N = p.get('N')

    inter_arr_times = iat
    assert len(iat) >= max_play
    srv_times = {}
    for i in range(N):
        srv_times['q{}'.format(i)] = srvt[i]
...
def run(**p):
    ...
    for j in range(len(beta_all)):
        ...
        CountingSimulatorInstance, inter_arr_times, service_times = \
            InferenceInstance.get_precomputed_CountingSimulator(inter_arrival_times=inter_arr_times, no_jobs=5000,
                                                                inference_type='gammaMixture', sim_iat=True)
        ...
        nr_threads = p.get('nr_threads')
        _worker = partial(worker, p=p, queuing_simulator=QueuingSimulator, output_dir=output_dir, j=j,
                          delay_rate=delay_rate, total_threads=nr_threads,
                          iat=inter_arr_times, srvt=service_times)

        with mp.Pool(processes=nr_threads, maxtasksperchild=1) as pool:
            pool.map(_worker, range(p.get('max_play_main')), chunksize=1)

        avg_compiler = Compile(output_dir, expected_iter=p.get('max_play_main'))
