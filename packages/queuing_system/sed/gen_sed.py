""" Solving the routing problem using sed algorithm till maximum number of plays """

import numpy as np
from packages.queuing_system.sed.sed_env import ModelExtractor
from packages.queuing_system import QueueingSystem


class Sed(QueueingSystem):
    def __init__(self, max_play: int, arrival_rate: int, shape, queues_lengths: np.ndarray, dept_rates: np.ndarray,
        ...
        QueueingSystem.__init__(self, max_play, arrival_rate, shape, queues_lengths, dept_rates, N,
                                discount, drop_reward, starting_st, delay_rate, ModelExtractor, 'Sed', srv_times,
                   inter_arr_times)
